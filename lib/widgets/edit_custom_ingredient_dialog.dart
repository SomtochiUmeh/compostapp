...
class EditCustomIngredientDialog extends StatefulWidget {
  ...
  @override
  State<EditCustomIngredientDialog> createState() =>
      _EditCustomIngredientDialogState();
}

class _EditCustomIngredientDialogState
    extends State<EditCustomIngredientDialog> {
  ...
  void _populateFields() {
    ...
    _priceController.text =
        widget.ingredient.price?.pricePerTon.toString() ?? '';

    // Populate nutrient fields - convert from decimal to percentage for display
    _dryMatterController.text =
        (widget.ingredient.nutrients.dryMatterPercent * 100).toString();
    _organicCarbonController.text =
        (widget.ingredient.nutrients.organicCarbonPercent * 100).toString();
    _nitrogenController.text =
        (widget.ingredient.nutrients.nitrogenPercent * 100).toString();
    _phosphorusController.text =
        (widget.ingredient.nutrients.phosphorusPercent * 100).toString();
    _potassiumController.text =
        (widget.ingredient.nutrients.potassiumPercent * 100).toString();
    _calciumController.text =
        (widget.ingredient.nutrients.calciumPercent * 100).toString();
    _magnesiumController.text =
        (widget.ingredient.nutrients.magnesiumPercent * 100).toString();
    _carbonNitrogenController.text = widget
        .ingredient.nutrients.carbonNitrogenRatio
        .toString(); // C:N ratio is not a percentage
  }
